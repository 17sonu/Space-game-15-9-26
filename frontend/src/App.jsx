...
function App() {
  ...
  const playLifeLostSound = useCallback(() => {
    const audio = lifeLostSoundRef.current;
  ...
  const saveScore = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.message || "Could not save score"
        );
      }
  ...
  const newGame = useCallback(() => {
    ...
    gameRef.current = {
      ...
      giftCollected: false,
      gift: null,
      giftTriggerRemaining: getRandomGiftTrigger(
        createEnemiesForLevel(1).length
      ),
  ...
  const startNextLevel = useCallback(() => {
    ...
    game.giftTriggerRemaining = getRandomGiftTrigger(
      game.enemies.length
    );
  ...
  useEffect(() => {
    ...
    function update(game, dt) {
      ...
      if (touchRef.current.active) {
        ...
        const touchStep =
          game.player.speed * 1.35 * dt;

        if (Math.abs(difference) <= touchStep) {
          game.player.x = targetX;
        } else {
          game.player.x +=
            Math.sign(difference) * touchStep;
        }
      } else {
        game.player.x +=
          direction * game.player.speed * dt;
      }

      game.player.x = Math.max(
        28,
        Math.min(W - 28, game.player.x)
      );
      ...
      if (
        levelData?.formationSpeed > 0 &&
        game.enemies.length
      ) {
        game.formationTime += dt;

        game.formationOffset +=
          game.formationDirection *
          levelData.formationSpeed *
          dt;

        const formationLimit =
          360 -
          ((levelData.cols - 1) *
            levelData.gapX) /
            2;
      ...
      for (
        ...
        for (
          ...
          if (
            ...
            // One gift per level.
            // Its trigger is randomized at level start.
            if (
              !game.giftSpawned &&
              game.enemies.length <=
                game.giftTriggerRemaining
            ) {
              createGift(game);
            }
      ...
      // Gift pickup:
      // The gift falls toward the player.
      // It can only appear once in each level
      // and increases the number of simultaneous shooters.
      if (game.gift) {
        game.gift.y +=
          game.gift.speed * dt;
    ...
    function draw(game) {
      ...
      ctx.font =
        "bold 28px 'Share Tech Mono', Consolas, monospace";
      ...
      ctx.font =
        "bold 22px 'Share Tech Mono', Consolas, monospace";
      ...
      ctx.font =
        "18px 'Share Tech Mono', Consolas, monospace";
      ...
      if (
        ...
        // Draw the jet's extra weapon barrels
        // so the power-up is visible.
        const shooterCount =
          Math.max(
            1,
            game.shooterCount || 1
      ...
      if (
        ...
        ctx.font =
          "bold 46px 'Share Tech Mono', Consolas, monospace";
        ...
        ctx.font =
          "20px 'Share Tech Mono', Consolas, monospace";
  ...
  return (
    <div className="app">
      ...
      {/* Life lost sound */}
      <audio
        ref={lifeLostSoundRef}
        src="/audio/life-lost.wav"
        preload="auto"
      />
